...
// decodeBlockV3Response turns a raw /eth/v3/validator/blocks response (SSZ or
// JSON) into a generic block.
...
// decodeBlockV4Response turns a raw /eth/v4/validator/blocks response into a
// generic block. When the response is SSZ with the payload included it also
// returns the decoded block contents so the caller can cache the envelope; the
// contents are nil otherwise. It is side-effect free so the block-freshness
// race can decode candidate responses to inspect their parent root.
