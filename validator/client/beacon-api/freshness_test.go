...
func TestFreshnessOptions_NoHintIsFirstSuccess(t *testing.T) {
	opts := freshnessOptions(context.Background(), attestationRootExtractor)
...
func TestBlockFreshnessOptions_WithHintBuildsOptions(t *testing.T) {
	...
	assert.Equal(t, 2, len(opts), "hint => ssz accept + deadline options")
}
...
func TestFreshnessOptions_WithHintBuildsOptions(t *testing.T) {
	...
	opts := freshnessOptions(ctx, attestationRootExtractor)
	assert.Equal(t, 3, len(opts), "polling hint with deadline => accept + deadline + repoll options")
	...
	optsNoPoll := freshnessOptions(ctx, syncBlockRootExtractor)
	assert.Equal(t, 2, len(optsNoPoll), "single-round read => accept + deadline options")

	// Without a deadline, only the accept option is produced.
	ctxNoDeadline := iface.WithHint(context.Background(), iface.Hint{Head: head})
	optsNoDeadline := freshnessOptions(ctxNoDeadline, attestationRootExtractor)
	assert.Equal(t, 1, len(optsNoDeadline))
}
...
// exercised end-to-end by the rest package's raceReadUntil tests.
func TestFreshnessOptions_PollPastDeadlineStillBounded(t *testing.T) {
	head := func() ([32]byte, primitives.Slot, bool) { return root32(0xaa), 10, true }
	ctx := iface.WithHint(context.Background(), iface.Hint{Head: head, Deadline: time.Now().Add(-time.Second)})

	opts := freshnessOptions(ctx, attestationRootExtractor)
	assert.Equal(t, 3, len(opts), "an already-elapsed poll deadline must still yield accept + (floored) deadline + repoll")
}
